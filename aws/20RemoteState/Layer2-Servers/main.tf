...
terraform {
  backend "s3" {
    bucket = "oleg-drachyshyn-terraform-state" // Bucket where to SAVE Terraform State
    key    = "dev/servers/terraform.tfstate"   // Object name in the bucket to SAVE Terraform State
    region = "eu-north-1"                      // Region where bycket created
  }
}
#====================================================================

# Read and change records from terraform.tfstate (work with)
data "terraform_remote_state" "network" {
  backend = "s3"
  config = {
    bucket = "oleg-drachyshyn-terraform-state" // Bucket from where to GET Terraform State
